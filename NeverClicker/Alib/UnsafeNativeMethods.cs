...
namespace Alib {
	/// <summary>
	/// These functions serve as a flat wrapper for Alib.dll.
	...
	internal class UnsafeNativeMethods {
		...
		/// <param name="Options">Additional parameter passed to Alib.dll (not available in Version 2 alpha).</param>
		/// <param name="Parameters">Parameters passed to dll.</param>
		/// <returns>	ahkdll returns a thread handle.</returns>
		/// <remarks>ahktextdll is available in Alib[Mini].dll only, not in Alib.exe.</remarks>
		...
		/// <remarks>Available in Alib[Mini].dll only, not in Alib.exe.</remarks>
		[DllImport(DLLPATH, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
		...
		/// <summary>
		/// ahkPause will pause/un-pause a thread and run traditional Alib Sleep internally.
		/// </summary>
