			...
			for (uint charIdx = 0; charIdx < charsMax; charIdx++) {
				...
				for (var taskId = 0; taskId < ProfessionTaskNames.Length; taskId++) {
					...
					if (DateTime.TryParse(intr.GameAccount.GetSettingOrEmptyString(settingKey, 
						...
						profTaskMatureTime = CalculateTaskMatureTime(mostRecent, charIdx, TaskKind.Professions, taskId);
						profTaskMatureTime = (profTaskMatureTime < now) ? now : profTaskMatureTime;

						this.Add(new GameTask(profTaskMatureTime, charIdx, TaskKind.Professions, taskId));
						tasksQueued += 1;
					}
				}
